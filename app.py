...
ALL_RADAR = {
    ...
    "xG vs Mål":       ("gMinusXG",  5,    False),
    "xP":              ("xP",          20,  False),
    "xP/90":           ("xPpx",       15,   False),
    "Ass. vs xA":      ("aMinusXA",   5,    False),
    "Brytningar/90":   ("intPer90",  4.0,  False),
    "Tacklingar/90":   ("tklWPer90", 4.0,  False),
    "Krossningar":     ("crs",       80,   False),
...
DEFAULT_RADAR = {
    ...
    "DF": ["Brytningar/90","Tacklingar/90","Krossningar","Mål/90","xA/90","xP/90"],
    "MF": ["Mål/90","Assist/90","xG/90","xA/90","xP/90","Skott/90","Brytningar/90"],
...
def scout_panel(p, season, show_avg_vals=True):
    ...
    if is_gk:
        ...
        xg_rows = []
        has_xp = p.get("xP",0) > 0 or p.get("xPpx",0) > 0
        if has_xg or has_xp:
            xg_rows = [
                row("xG",             "xG",       20,   False),
...
if "IFK Göteborg" in view:
    ...
    pos_local = st.radio("", ["Alla","GK","DF","MF","FW"],
                          horizontal=True, label_visibility="collapsed", key="ifk_pos")
...
# ══ VIEW: JÄMFÖR ═════════════════════════════════════════════════════════════
    ...
    if len(pdata) < 2:
        ...
        ALL_CMP_STATS = [
            ...
            ("Krossningar",     "crs",       lambda v:str(int(v))),
